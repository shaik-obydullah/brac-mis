...
@section('title', 'Create Returnee')

@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-800">Create Returnee</h1>
    <p class="text-gray-600 mt-1">Register a returning migrant</p>
</div>

<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
    <form action="{{ route('returnees.store') }}" method="POST">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="migrant_id" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-orange-500 focus:border-orange-500">
                    ...
                    @foreach($migrants as $migrant)
                        <option value="{{ $migrant->id }}" {{ old('migrant_id') == $migrant->id ? 'selected' : '' }}>{{ $migrant->name }} ({{ $migrant->brac_id ?? $migrant->id }})</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="beneficiary_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-orange-500 focus:border-orange-500">
                    ...
                    @foreach($beneficiaries ?? [] as $beneficiary)
                        <option value="{{ $beneficiary->id }}" {{ old('beneficiary_id') == $beneficiary->id ? 'selected' : '' }}>{{ $beneficiary->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="date" name="return_date" value="{{ old('return_date') }}" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-orange-500 focus:border-orange-500">
            ...
            <div>
                ...
                <select name="origin_country_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-orange-500 focus:border-orange-500">
                    ...
                    @foreach($countries ?? [] as $country)
                        <option value="{{ $country->id }}" {{ old('origin_country_id') == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="return_reason" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-orange-500 focus:border-orange-500">{{ old('return_reason') }}</textarea>
            ...
            <div>
                ...
                <select name="current_status" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-orange-500 focus:border-orange-500">
                    ...
                    <option value="assessed" {{ old('current_status') == 'assessed' ? 'selected' : '' }}>Assessed</option>
                    <option value="planning" {{ old('current_status') == 'planning' ? 'selected' : '' }}>Planning</option>
                    <option value="in_progress" {{ old('current_status') == 'in_progress' ? 'selected' : '' }}>In Progress</option>
                    <option value="completed" {{ old('current_status') == 'completed' ? 'selected' : '' }}>Completed</option>
                    <option value="dropped" {{ old('current_status') == 'dropped' ? 'selected' : '' }}>Dropped</option>
                </select>
        ...
        <div class="flex items-center gap-4 mt-8 pt-6 border-t border-gray-200">
            <button type="submit" class="bg-orange-600 hover:bg-orange-700 text-white px-6 py-2.5 rounded-lg font-medium">Save Returnee</button>
